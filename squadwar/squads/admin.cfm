	 	...
	 	<center>
	 		<table width="95%" cellpadding=0 cellspacing=0 border=0><tr><td>	
				...
				<div class="title">Squad Login</div>
						...
						<div class="copy">
							<cfform action="squadlogin_validate.cfm" method="POST" enablecab="no" name="login">
								<b>Squad:</b><br>
								<select name="id">
									<cfoutput query="get_squad">
										<option value="#SquadID#"<cfif isdefined("session.adminlastchosen")><cfif squadid IS session.adminlastchosen> selected</cfif></cfif>>#SquadName#
									</cfoutput>
								</select>
								<p>
