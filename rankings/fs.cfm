...
<!--- Page info goes here	 --->			
<font color="white"><b><div class="newsitemtitle">Welcome to the FreeSpace rankings page!</div></b></font>

<cfset locked=0>
<cfif locked>
	<p>
	<div class=copy>The FreeSpace rankings pages have been temporarily disabled. Thank you for your patience.</div>
...
<cfif Trim("type") IS ''><cfset type="scores"></cfif>
	<cfif type IS 'kills'>
		<cfif isdefined("application.fskillsmax")><cfset app_fskillsmax=application.fskillsmax><cfelse><cfset app_fskillsmax=0></cfif>
			...
			FROM         Freespace 
			WHERE 		KillCountOK > #app_fskillsmax#
			ORDER BY  KillCountOK DESC
			</CFQUERY>
			<cfloop query="fsscores" startrow="#Evaluate(maxscore + 99)#" endrow="#Evaluate(maxscore + 100)#">
				<cfset application.fskillsmax=fsscores.KillCountOK>
			</cfloop>			
	<cfelse><cfif type IS 'msns'>
			<cfif isdefined("application.fsmsnsmax")><cfset app_fsmsnsmax=application.fsmsnsmax><cfelse><cfset app_fsmsnsmax=0></cfif>
			<CFQUERY NAME="fsscores" DATASOURCE="PXO" cachedwithin="#CreateTimeSpan(0,0,15,0)#">
			SELECT      TrackerID, Rank, Pilot, Score, KillCountOK, MissionsFlown, FlightTime
			FROM         Freespace 
	...
	<cfelse><cfif type IS 'time'>
			<cfif isdefined("application.fstimemax")><cfset app_fstimemax=application.fstimemax><cfelse><cfset app_fstimemax=0></cfif>
			<CFQUERY NAME="fsscores" DATASOURCE="PXO" cachedwithin="#CreateTimeSpan(0,0,15,0)#">
			SELECT      TrackerID, Rank, Pilot, Score, KillCountOK, MissionsFlown, FlightTime
			FROM         Freespace 
			WHERE 		FlightTime > #app_fstimemax#
	...
	<cfelse>
			<cfif isdefined("application.fsscoresmax")><cfset app_fsscoresmax=application.fsscoresmax><cfelse><cfset app_fsscoresmax=0></cfif>
			<CFQUERY NAME="fsscores" DATASOURCE="PXO" cachedwithin="#CreateTimeSpan(0,0,15,0)#">
			SELECT      TrackerID, Rank, Pilot, Score, KillCountOK, MissionsFlown, FlightTime
			FROM         Freespace 
			WHERE 		Score > #app_fsscoresmax#
			ORDER BY  Score DESC
			</CFQUERY>
			<cfloop query="fsscores" startrow="#Evaluate(maxscore + 99)#" endrow="#Evaluate(maxscore + 100)#">
